// Top‑10 desert well seed finder – interactive tuning, arbitrary formations
// Compile: g++ -Ofast -march=native -mtune=native -flto -DNDEBUG -pthread -o finder finder.cpp
#include <bits/stdc++.h>
#include <atomic>
#include <chrono>
#include <csignal>
...
// ---------- Mask structures ---------------------------------------------------
struct alignas(64) RM1024 { uint64_t w[32]{}; };   // holds up to 2048 bits
struct alignas(32) M1024 {
    __m256i v[4];                                 // 1024 bits (up to 10 low bits)
...
static void printProgress(const string& phaseName, uint64_t processed, uint64_t total, double rate) {
    ...
    cerr << '[' << phaseName << "] " << fixed << setprecision(3) << pct << "% (" << processed << '/' << total << ") rate=" << rate << "/s";
    if (phaseName == "search") cerr << " found=" << foundCount << " candidates=" << candidateCount;
    cerr << '\n';
}
...
// ---------- Base scanning ------------------------------------------------------
static void scanAllBases(bool testMode) {
    uint64_t limit = TOTAL_UINT32;
    Phase baseStatus{ "base scan", limit };
    unsigned threadCount = thread::hardware_concurrency(); if (threadCount == 0)threadCount = 1;
    atomic<bool> timeLimitReached{ false };
    if (testMode) thread([&]() { this_thread::sleep_for(seconds(10)); timeLimitReached = true; stopRequested = true; }).detach();

    auto worker = [&](unsigned tid) {
        uint64_t block = (limit + threadCount - 1) / threadCount, start = tid * block, end = min(start + block, limit);
        vector<vector<uint32_t>> local(criteria.size());
        uint64_t proc = 0;
        for (uint64_t v = start; v < end && !timeLimitReached; ++v) {
            uint32_t base = uint32_t(v);
            uint32_t regionSeed = computeRegionSeedFromBase(base);
            DW::RNG rng(regionSeed);
            if (rng.next<500>() != 0u) { ++proc; if ((proc & (FLUSH_INTERVAL - 1)) == 0) { baseStatus.processed += proc; proc = 0; } continue; }
            int offX = rng.next<16>(), offZ = rng.next<16>();
            for (size_t i = 0; i < criteria.size(); ++i) if (offsetInCriteria(offX, offZ, criteria[i])) local[i].push_back(base);
            ++proc; if ((proc & (FLUSH_INTERVAL - 1)) == 0) { baseStatus.processed += proc; proc = 0; }
        }
        baseStatus.processed += proc;
        lock_guard<mutex> g(outputMutex);
        for (size_t i = 0; i < local.size(); ++i) baseSets[i].insert(baseSets[i].end(), local[i].begin(), local[i].end());
        };
    vector<thread> threads; for (unsigned t = 0; t < threadCount; ++t) threads.emplace_back(worker, t);
    thread rep(reporter, cref(baseStatus));
    for (auto& t : threads) t.join();
    stopRequested = true; if (rep.joinable())rep.join(); stopRequested = false;
    for (auto& s : baseSets) { sort(s.begin(), s.end()); s.erase(unique(s.begin(), s.end()), s.end()); }
    cerr << "Base scan finished. Set sizes:"; for (auto& s : baseSets)cerr << ' ' << s.size(); cerr << '\n';
}
...
// ---------- Interactive tuning -----------------------------------------------
// ---------- Interactive tuning (FIXED BENCHMARK) -------------------------------
    ...
    while (true) {
        ...
        if (iss >> hb >> lb) {
            ...
            if (iss >> tlim) {
                // --- Benchmark mode ---
                if (tlim <= 0.0) {
                    cerr << "Time limit must be positive.\n";
                    continue;
                }
                ...
                MITM_LOW_BITS = lb;
                MITM_HIGH_SIZE = 1 << hb;
                MITM_HIGH_MASK = MITM_HIGH_SIZE - 1;
                MITM_LOW_SIZE = 1 << lb;
                MITM_LOW_MASK = MITM_LOW_SIZE - 1;
                LOWER_SIZE = 1 << LOWER_BITS;
                LOWER_MASK = LOWER_SIZE - 1;
                ...
                thread timer([&]() {
                    this_thread::sleep_for(duration<double>(tlim));
                    timeUp = true;
                    });
                ...
                while (!timeUp) {
                    ...
                    // Pre‑permute rows for this sl10 (same as real search)
                    ...
                    for (uint32_t row0 = 0; row0 < MITM_HIGH_SIZE && !timeUp; ++row0) {
                        ...
                        // Extract bits and perform REAL candidate verification (no isWell)
                        ...
                        for (int w = 0; w < 16 && !timeUp; ++w) {
                            ...
                            while (bits && !timeUp) {
                                ...
                                // Simulate the real verification cost (binary searches)
                                for (uint32_t k = 0; k < cnt && !timeUp; ++k) {
                                    ...
                                    for (size_t o = 1; o < baseSets.size(); ++o) {
                                        ...
                                        if (!binary_search(baseSets[o].begin(), baseSets[o].end(), targetBase)) {
                                            ok = false;
                                            break;
                                        }
                                    }
                                    if (ok) {
                                        // Valid candidate found (we do not call nearest/isWell to save time)
                ...
                if (seedsDone == 0) {
                    cout << "No seeds processed (time limit too short?).\n";
                }
                else {
                    double us_per_seed = elapsed * 1e6 / seedsDone;
                    cout << seedsDone << " seeds in " << elapsed << " s (" << us_per_seed << " µs/seed)\n";
                }
            }
            else {
                // Finalise split (two integers)
                LOWER_BITS = totalBits;
                MITM_HIGH_BITS = hb;
                MITM_LOW_BITS = lb;
                MITM_HIGH_SIZE = 1 << hb;
...
static void addSolution(const BestSol& sol) {
    ...
    // Check for exact duplicate (same seed, xMul, zMul, baseO)
    for (const auto& t : topSolutions) {
        if (t.seed == sol.seed && t.xMul == sol.xMul && t.zMul == sol.zMul && t.baseO == sol.baseO)
            return;
    }
...
// ---------- Search phase --------------------------------------------------------
static void searchWorker(unsigned tid, uint64_t limitTotal, unsigned threadCount,
    bool testMode, steady_clock::time_point searchStart, Phase& searchStatus) {
        ...
        for (uint64_t base = start; base < end; base += MITM_LOW_SIZE) {
            ...
            for (uint32_t row0 = 0; row0 < MITM_HIGH_SIZE; ++row0) {
                ...
                for (size_t o = 1; o < baseSets.size(); ++o) {
                    ...
                    auto [wordShift, bitShift, bitShift2] = getShift(dLow10, MITM_LOW_MASK);
                    bool doShift = (bitShift != 0);
                    __m128i shiftR = _mm_cvtsi32_si128(bitShift);
                    __m128i shiftL = _mm_cvtsi32_si128(bitShift2);
                ...
                for (int w = 0; w < 16; ++w) {
                    ...
                    while (bits) {
                        ...
                        for (uint32_t k = 0; k < cnt; ++k) {
                            ...
                            addSolution(sol);   // <-- store in top‑10 list
                        }
                    }
                }
            }
...
int main() {
    ...
    cout << "Enter mode (1 = normal, 2 = test): " << flush; int mode; cin >> mode;
    bool testMode = (mode == 2);

    scanAllBases(testMode);
    ...
    for (unsigned t = 0; t < threadCount; ++t)
        searchThreads.emplace_back([&, t]() { searchWorker(t, limitTotal, threadCount, testMode, searchStart, searchStatus); });
    thread rep(reporter, cref(searchStatus));
    for (auto& t : searchThreads) t.join();
    stopRequested = true; if (rep.joinable()) rep.join();

    if (!topSolutions.empty()) {
        cerr << "Top " << topSolutions.size() << " solutions (by chunk distance):\n";
        for (size_t i = 0; i < topSolutions.size(); ++i) {
            const auto& sol = topSolutions[i];
            cerr << " #" << i + 1 << " seed=" << sol.seed
                << " xMul=" << sol.xMul << " zMul=" << sol.zMul
                << " originChunk=(" << sol.chunkX << ',' << sol.chunkZ << ')'
